...
public class C2CRechargePaymentTimeoutCustomer {

    @Resource
    private C2COrderHandler c2COrderHandler;

    @RabbitListener(queues = QueueConstant.C2C_ORDER_PAYMENT_TIMEOUT_PROGRESS, containerFactory = "reportRabbitListener")
    public void receiveNotice(Channel channel, Message message) throws IOException {
        ...
        try {
            c2COrderHandler.submitPaymentTimeout(rechargeOrderId);
            channel.basicAck(deliveryTag, false);
        } catch (Exception ex) {
            log.error("C2C充值订单{}付款超时检查时发生异常", rechargeOrderId, ex);
        }
    }
