...
class MachiScript_Recharge {

    private static final Logger log = LoggerFactory.getLogger(MachiScript_Recharge.class)
    ...
    private void prepareToWangyin(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount account, RechargePrepareDO req, GlRechargeResult result) {
        ...
        paramMap.put("txnAmt", req.getAmount().multiply(BigDecimal.valueOf(100)).setScale(0).toString())
        //充值金额 *100 单位为分
        paramMap.put("currencyCode", "156")    //交易币种 默认156
        paramMap.put("cardType", "DT01")    // 卡类型 暂只支持借记卡：DT01
        paramMap.put("bankNum", glPaymentChannelBankBusiness.getBankCode(req.getBankId(), account.getChannelId()))
        // 银行代码
