package com.seektop.fund.payment.gbopay
...
class GBOScript {
    ...
    private static final Logger log = LoggerFactory.getLogger(GBOScript.class)
    ...
    private void prepareScan(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount payment, RechargePrepareDO req, GlRechargeResult result, String payType, Object[] args) {
        try {
            ...
            DataContentParms.put("name_goods", req.getFromCardUserName())
            ...
            DataContentParms.put("return_type", "card")

            String toSign = MD5.toAscii(DataContentParms) + payment.getPrivateKey()
            DataContentParms.put("sign", MD5.md5(toSign))

            log.info("GBOScript_Prepare_Params:{}", JSON.toJSONString(DataContentParms))
            ...
            String restr = okHttpUtil.post(payment.getPayUrl() + "/gateway/bankgateway/pay", DataContentParms, requestHeader)

            JSONObject json = JSON.parseObject(restr);
            log.info("GBOScript_Prepare_resStr:{}", json)
            ...
            String bankName = json.getString("bank_name")
            String cardNo = json.getString("card_no")
            String cardUser = json.getString("card_user")

            this.glPaymentChannelBankBusiness = com.seektop.fund.payment.groovy.BaseScript.getResource(args[0], ResourceEnum.GlPaymentChannelBankBusiness) as GlPaymentChannelBankBusiness
            GlPaymentChannelBank bank = glPaymentChannelBankBusiness.getBank(payment.getChannelId(), bankName)
            result.setErrorCode(FundConstant.RechargeErrorCode.NORMAL)
            BankInfo bankInfo = new BankInfo();
            bankInfo.setName(cardUser)
            if (null != bank) {
    ...
    RechargeNotify notify(Object[] args) throws GlobalException {
        ...
        log.info("GBOScript_Notify_resMap:{}", JSON.toJSONString(resMap))
    ...
    RechargeNotify payQuery(Object[] args) throws GlobalException {
        ...
        log.info("GBOScript_Query_reqMap:{}", JSON.toJSONString(DataContentParms))
        GlRequestHeader requestHeader =
                getRequestHeard("", "", orderId, GlActionEnum.RECHARGE_QUERY.getCode(), account.getChannelId(), account.getChannelName())
        String resStr = okHttpUtil.post(account.getPayUrl() + "/check/checkorder/checkorderresult", DataContentParms, requestHeader)
        log.info("GBOScript_Query_resStr:{}", resStr)
    ...
    WithdrawResult withdraw(Object[] args) throws GlobalException {
        ...
        log.info("GBOScript_Transfer_params: {}", JSON.toJSONString(DataContentParms))
        GlRequestHeader requestHeader =
        ...
        String resStr = okHttpUtil.post(merchantAccount.getPayUrl() + "/gateway/pay", DataContentParms, requestHeader)
        log.info("GBOScript_Transfer_resStr: {}", resStr)
    ...
    WithdrawNotify withdrawNotify(Object[] args) throws GlobalException {
        ...
        this.glPaymentChannelBankBusiness = com.seektop.fund.payment.groovy.BaseScript.getResource(args[3], ResourceEnum.GlPaymentChannelBankBusiness) as GlPaymentChannelBankBusiness
        log.info("GBOScript_WithdrawNotify_resMap:{}", JSON.toJSONString(resMap))
        JSONObject json = JSONObject.parseObject(resMap.get("reqBody"))
        String orderid = json.getString("no_order")
    ...
    WithdrawNotify withdrawQuery(Object[] args) throws GlobalException {
        ...
        log.info("GBOScript_TransferQuery_reqMap:{}", JSON.toJSONString(DataContentParms))
        GlRequestHeader requestHeader =
                getRequestHeard("", "", orderId, GlActionEnum.WITHDRAW_QUERY.getCode(), merchant.getChannelId(), merchant.getChannelName())
        String resStr = okHttpUtil.post(merchant.getPayUrl() + "/gateway/pay/queryOrder", DataContentParms, requestHeader)
        log.info("GBOScript_TransferQuery_resStr:{}", resStr)
    ...
    BigDecimal balanceQuery(Object[] args) throws GlobalException {
        ...
        log.info("GBOScript_QueryBalance_reqMap: {}", JSON.toJSONString(DataContentParms))
        GlRequestHeader requestHeader =
                getRequestHeard("", "", "", GlActionEnum.PAYMENT_QUERY_BALANCE.getCode(), merchantAccount.getChannelId(), merchantAccount.getChannelName())
        String resStr = okHttpUtil.post(merchantAccount.getPayUrl() + "/gateway/pay/queryAmount", DataContentParms, requestHeader)
        log.info("GBOScript_QueryBalance_resStr: {}", resStr)
