...
class HongHuScript_recharge {

    private static final Logger log = LoggerFactory.getLogger(HongHuScript_recharge.class)
    ...
    private void prepareScan(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount payment, RechargePrepareDO req, GlRechargeResult result, String payType) {
        try {
            ...
            log.info("HongHuScript_Prepare_Params:{}", JSON.toJSONString(DataContentParms))
            ...
            String restr = okHttpUtil.post(payment.getPayUrl() + "/gateway/bankgateway/pay", DataContentParms, 30L, requestHeader)
            log.info("HongHuScript_Prepare_resStr:{}", restr)
    ...
    RechargeNotify notify(Object[] args) throws GlobalException {
        ...
        log.info("HongHuScript_Notify_resMap:{}", JSON.toJSONString(resMap))
    ...
    RechargeNotify payQuery(Object[] args) throws GlobalException {
        ...
        log.info("HongHuScript_Query_reqMap:{}", JSON.toJSONString(DataContentParms))
        ...
        String resStr = okHttpUtil.post(account.getPayUrl() + "/check/checkorder/checkorderresult", DataContentParms, 30L, requestHeader)
        log.info("HongHuScript_Query_resStr:{}", resStr)
