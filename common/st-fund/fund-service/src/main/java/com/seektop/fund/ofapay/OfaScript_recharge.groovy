package com.seektop.fund.ofapay
...
class OfaScript_recharge {

    private static final Logger log = LoggerFactory.getLogger(OfaScript_recharge.class)
    ...
    private void prepareScan(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount payment, RechargePrepareDO req, GlRechargeResult result, String paytype) {
        ...
        DataContentParms.put("scode", payment.getMerchantCode())
        ...
        DataContentParms.put("productname", "book")
        DataContentParms.put("currency", "CNY")
        DataContentParms.put("userid", req.getUserId().toString())
        DataContentParms.put("noticeurl", payment.getNotifyUrl() + merchant.getId())
        ...
        log.info("OFAScript_Prepare_Params:{}", JSON.toJSONString(DataContentParms))
        ...
        String restr = okHttpUtil.postJSON(payment.getPayUrl() + "/pay/order.aspx", JSON.toJSONString(DataContentParms),  requestHeader)
        log.info("OFAScript_Prepare_resStr:{}", restr)
    ...
    RechargeNotify notify(Object[] args) throws GlobalException {
        ...
        log.info("OFAScript_Notify_resMap:{}", JSON.toJSONString(resMap))
        JSONObject json = JSON.parseObject(resMap.get("reqBody"))
        String orderid = json.getString("orderid")
        String thirdOrderId = json.getString("orderno")
    ...
    RechargeNotify payQuery(Object[] args) throws GlobalException {
        ...
        Map<String, String> DataContentParms = new HashMap<String, String>()
        DataContentParms.put("scode", account.getMerchantCode())
        DataContentParms.put("orderid", orderId)

        String toSign = MD5.toAscii(DataContentParms) + "&" + account.getPrivateKey()
        DataContentParms.put("sign", MD5.md5(toSign))

        log.info("OFAScript_Query_reqMap:{}", JSON.toJSONString(DataContentParms))
        ...
        String resStr = okHttpUtil.postJSON(account.getPayUrl() + "/pay/query.aspx", JSON.toJSONString(DataContentParms),  requestHeader)
        log.info("OFAScript_Query_resStr:{}", resStr)

        JSONObject json = JSON.parseObject(resStr)

        //支付状态 0:處理中  1:交易成功   -1:交易或請求失敗
