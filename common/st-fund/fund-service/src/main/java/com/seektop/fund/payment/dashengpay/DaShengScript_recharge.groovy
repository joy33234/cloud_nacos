...
class DaShengScript_recharge {

    private static final Logger log = LoggerFactory.getLogger(DaShengScript_recharge.class)
    ...
    private void prepareScan(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount payment, RechargePrepareDO req, GlRechargeResult result, String payType) {
        ...
        if (json.getString("ret_code") != "0000"
                || StringUtils.isEmpty(json.getString("card_no"))) {
            result.setErrorCode(FundConstant.RechargeErrorCode.PAYMENT)
            result.setErrorMsg("商户异常")
            return
        }
        BankInfo bankInfo = new BankInfo()
        bankInfo.setName(json.getString("card_user"))
        bankInfo.setBankId(-1)
        bankInfo.setBankName(json.getString("bank_name"))
        bankInfo.setCardNo(json.getString("card_no"))
