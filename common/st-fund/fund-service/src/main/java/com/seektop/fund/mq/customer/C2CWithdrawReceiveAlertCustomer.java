...
public class C2CWithdrawReceiveAlertCustomer {

    @Resource
    private C2COrderHandler c2COrderHandler;

    @RabbitListener(queues = QueueConstant.C2C_ORDER_RECEIVE_ALERT_PROGRESS, containerFactory = "reportRabbitListener")
    public void receiveNotice(Channel channel, Message message) throws IOException {
        ...
        try {
            c2COrderHandler.submitReceiveAlert(withdrawOrderId);
            channel.basicAck(deliveryTag, false);
        } catch (Exception ex) {
            log.error("C2C提现订单{}收款提醒检查时发生异常", withdrawOrderId, ex);
        }
    }
