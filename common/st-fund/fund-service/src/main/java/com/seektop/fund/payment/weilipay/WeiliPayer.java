...
@Slf4j
@Service(FundConstant.PaymentChannel.WEILIFU + "")
public class WeiliPayer implements GlPaymentRechargeHandler, GlPaymentWithdrawHandler {
    ...
    public void prepareToWangyinOrQuick(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount account, RechargePrepareDO req, GlRechargeResult result) {
        ...
        //充值金额单位为分
        param.put("txnAmt", req.getAmount().multiply(BigDecimal.valueOf(100)).setScale(0, RoundingMode.DOWN).toString());
        ...
        log.info("weilifu_recharge_prepare_wangyinOrQuick_params:{}", JSON.toJSONString(param));
        ...
        log.info("weilifu_recharge_prepare_wangyinOrQuick_result:{}", retBack);
        //正常请求返回html,有错误请求返回json字符串
        boolean isJsonStr = isJsonString(retBack);
        if (true == isJsonStr) {
            throw new RuntimeException("weilifu创建订单失败");
        }
    ...
    public void prepareToScan(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount account, RechargePrepareDO req, GlRechargeResult result) {
        ...
        String signStrTemp = MD5.toAscii(param) + "&k=" + keyValue;
        param.put("mac", MD5.md5(signStrTemp));
        log.info("weilifu_recharge_prepare_scan_params:{}", JSON.toJSONString(param));
        GlRequestHeader requestHeader = GlRequestHeader.builder()
                .action(GlActionEnum.RECHARGE.getCode())
                .channelId(PaymentMerchantEnum.WEILI_PAY.getCode() + "")
                .channelName(PaymentMerchantEnum.WEILI_PAY.getPaymentName())
                ...
                .build();
        String retBack = okHttpUtil.post(account.getPayUrl() + "/powerpay-gateway-onl/txn", param, requestHeader);
        log.info("weilifu_recharge_prepare_scan_result:{}", retBack);
    ...
    public void prepareToH5(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount account, RechargePrepareDO req, GlRechargeResult result) {
        ...
        String signStrTemp = MD5.toAscii(param) + "&k=" + keyValue;
        param.put("mac", MD5.md5(signStrTemp));
        log.info("weilifu_recharge_prepare_H5_params:{}", JSON.toJSONString(param));
        GlRequestHeader requestHeader = GlRequestHeader.builder()
                .action(GlActionEnum.RECHARGE.getCode())
                .channelId(PaymentMerchantEnum.WEILI_PAY.getCode() + "")
                .channelName(PaymentMerchantEnum.WEILI_PAY.getPaymentName())
                ...
                .build();
        String retBack = okHttpUtil.post(account.getPayUrl() + "/powerpay-gateway-onl/txn", param, requestHeader);
        log.info("weilifu_recharge_prepare_H5_result:{}", retBack);
        JSONObject retJson = JSONObject.parseObject(retBack);
        if (null == retJson || StringUtils.isEmpty(retJson.getString("codePageUrl"))) {
            throw new RuntimeException("weilifuH5创建订单失败");
        }
    ...
    public RechargeNotify notify(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount account, Map<String, String> resMap) throws GlobalException {
        log.info("weilipayer_Notify_resMap:{}", resMap);
        String orderId = resMap.get("orderId");
        String respCode = resMap.get("respCode");
        String txnStatus = resMap.get("txnStatus");
        if (StringUtils.isNotEmpty(respCode) && "0000".equals(respCode) && StringUtils.isNotEmpty(txnStatus) && "10".equals(txnStatus)) {
    ...
    public RechargeNotify query(GlPaymentMerchantaccount account, String orderId) throws GlobalException {
        ...
        String signStrTemp = MD5.toAscii(param) + "&k=" + keyValue;
        param.put("mac", MD5.md5(signStrTemp));
        log.info("weili_recharge_query_params:{}", JSON.toJSONString(param));
        GlRequestHeader requestHeader = GlRequestHeader.builder()
                ...
                .build();
        String retBack = okHttpUtil.post(account.getPayUrl() + "/powerpay-gateway-onl/txn", param, requestHeader);
        log.info("weili_recharge_query_result:{}", retBack);
    ...
    public WithdrawResult doTransfer(GlWithdrawMerchantAccount merchantAccount, GlWithdraw req) throws GlobalException {
        ...
        param.put("orderId", req.getOrderId());
        //金额单位为分
        param.put("txnAmt", req.getAmount().subtract(req.getFee()).multiply(BigDecimal.valueOf(100)).setScale(0, RoundingMode.DOWN).toString());
        param.put("currencyCode", "156");
        ...
        log.info("weiliPayer_Transfer_OrderId:{},data:{}", req.getOrderId(), JSON.toJSONString(param));
        GlRequestHeader requestHeader = GlRequestHeader.builder()
        ...
        log.info("weiliPayer_Transfer_OrderId:{},response: {}", req.getOrderId(), retBack);
        ...
        if (!respCode.equals("0000") && !respCode.equals("1101")) {
            result.setValid(false);
            result.setMessage(json.getString("respMsg"));
            return result;
        }
        if (!txnStatus.equals("01") && !txnStatus.equals("10")) {
            result.setValid(false);
            result.setMessage(json.getString("respMsg"));
            return result;
        }
    ...
    public WithdrawNotify doTransferNotify(GlWithdrawMerchantAccount merchant, Map<String, String> resMap) throws GlobalException {
        log.info("weilipayer_doTransferNotify_resMap:{}", resMap);
        // 修改为验签模式 20191015
        if ("0000".equals(resMap.get("respCode")) || "1101".equals(resMap.get("respCode"))) {
            // 接口成功返回  验证 业务状态是否成功
            String mac = resMap.remove("mac");
            resMap.remove("reqBody");
            String signStrTemp = toAscii(resMap) + "&k=" + merchant.getPrivateKey();
            if (mac.equals(MD5.md5(signStrTemp))) {
                log.info("weilipayer_doTransferNotify_sign_success:{}", resMap.get("txnStatus"));
                ...
                if ("10".equals(resMap.get("txnStatus"))) {//成功
                    notify.setStatus(0);
                } else if ("20".equals(resMap.get("txnStatus"))) {
                    notify.setStatus(1);
                } else {
                    notify.setStatus(2);
                }
...
/*
    ...
    public WithdrawNotify doTransferQuery(GlWithdrawMerchantAccount merchant, String orderId) throws GlobalException {
        ...
        String signStrTemp = MD5.toAscii(param) + "&k=" + keyValue;
        param.put("mac", MD5.md5(signStrTemp));
        log.info("weili_Transfer_query_params:{}", JSON.toJSONString(param));
        GlRequestHeader requestHeader = GlRequestHeader.builder()
        ...
        log.info("weili_Transfer_query_result:{}", retBack);

        JSONObject retJson = JSONObject.parseObject(retBack);
        String respCode = retJson.getString("respCode");
        String txnStatus = retJson.getString("txnStatus");
        if (StringUtils.isNotEmpty(respCode) && "0000".equals(respCode)) {
            WithdrawNotify notify = new WithdrawNotify();
            ...
            if (StringUtils.isNotEmpty(txnStatus) && "10".equals(txnStatus)) {//成功
                notify.setStatus(0);
            } else if (StringUtils.isNotEmpty(txnStatus) && "20".equals(txnStatus)) {
                notify.setStatus(1);
            } else {
                notify.setStatus(2);
            }
    ...
    public BigDecimal queryBalance(GlWithdrawMerchantAccount merchantAccount) throws GlobalException {
        ...
        GlRequestHeader requestHeader = GlRequestHeader.builder()
                ...
                .build();
        String retBack = okHttpUtil.post(merchantAccount.getPayUrl() + "/powerpay-gateway-onl/txn", param, requestHeader);
        log.info("weilipayer_QueryBalance_resStr:{}", retBack);
        ...
        if (StringUtils.isNotEmpty(respCode) && respCode.equals("0000")) {
