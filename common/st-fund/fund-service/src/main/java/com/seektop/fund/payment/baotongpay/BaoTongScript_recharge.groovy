package com.seektop.fund.payment.baotongpay
...
class BaoTongScript_recharge {

    private static final Logger log = LoggerFactory.getLogger(BaoTongScript_recharge.class)
    ...
    private void prepareScan(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount payment, RechargePrepareDO req, GlRechargeResult result, String payType) {
        try {
            ...
            DataContentParms.put("pay_name", req.getFromCardUserName())

            String toSign = MD5.toAscii(DataContentParms) + payment.getPrivateKey()
            DataContentParms.put("sign", MD5.md5(toSign))

            log.info("BaoTongScript_Prepare_Params:{}", JSON.toJSONString(DataContentParms))
            ...
            String restr = okHttpUtil.post(payment.getPayUrl() + "/gateway/bk_gateway/pay", DataContentParms, 30L, requestHeader)
            log.info("BaoTongScript_Prepare_resStr:{}", restr)
            ...
            if (json.getString("ret_code") != "0000") {
                ...
                result.setErrorMsg("请求失败:" + json.getString("ret_msg"))
                return
            }

            if (StringUtils.isEmpty(json.getString("redirect_url"))) {
                result.setErrorCode(FundConstant.RechargeErrorCode.PAYMENT)
    ...
    RechargeNotify notify(Object[] args) throws GlobalException {
        ...
        log.info("BaoTongScript_Notify_resMap:{}", JSON.toJSONString(resMap))
    ...
    RechargeNotify payQuery(Object[] args) throws GlobalException {
        ...
        log.info("BaoTongScript_Query_reqMap:{}", JSON.toJSONString(DataContentParms))
        ...
        String resStr = okHttpUtil.post(account.getPayUrl() + "/gateway/bk_gateway/checkorderresult", DataContentParms, 30L, requestHeader)
        log.info("BaoTongScript_Query_resStr:{}", resStr)
