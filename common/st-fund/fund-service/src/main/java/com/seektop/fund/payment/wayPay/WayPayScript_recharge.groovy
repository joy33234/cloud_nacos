 ...
 * @auth otto
 */
public class WayPayScript_recharge {

    private static final Logger log = LoggerFactory.getLogger(WayPayScript_recharge.class)
    private OkHttpUtil okHttpUtil

    private  final String PAY_URL =  "/Apipay"  //所有接口网址一样
    private  final String QUERY_URL =  "/Apipay"
    ...
    private void prepareToScan(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount account, RechargePrepareDO req, GlRechargeResult result, String gateway) throws GlobalException {
        ...
        String notifyurl = account.getNotifyUrl() + merchant.getId() ;
        ...
        params.put("backurl"  , account.getNotifyUrl())
        params.put("userip" , req.getIp())
        params.put("paytype", gateway)

        String toSign =  account.getMerchantCode() +req.getOrderId() + amount + notifyurl +  account.getPrivateKey()
        params.put("sign",  MD5.md5(toSign))

        log.info("WayPayScript_recharge_prepare_params:{}", JSON.toJSONString(params))
        ...
        log.info("WayPayScript_recharge_prepare_resp:{}", resStr)
        ...
        if ( 1 != json.getInteger("status")  ) {
            result.setErrorCode(FundConstant.RechargeErrorCode.PAYMENT)
            result.setErrorMsg(json.getString("error"))
            return
        }
    ...
    public RechargeNotify notify(Object[] args) throws GlobalException {
        ...
        log.info("WayPayScript_notify_resp:{}", resMap)

        String thirdSign = resMap.get("currency") + resMap.get("status") + resMap.get("userid") +  resMap.get("orderno") + resMap.get("amount") + account.getPrivateKey() ;
        thirdSign = MD5.md5(thirdSign)

        if (StringUtils.isNotEmpty(resMap.get("orderno"))  &&  thirdSign == resMap.get("sign") ) {
            return this.payQuery(okHttpUtil, account, resMap.get("orderno") ) ;

        } else{
            log.info("WayPayScript_notify_Sign: 回调资料错误或验签失败，单号：{}" , resMap.get("orderno"))
    ...
    public RechargeNotify payQuery(Object[] args) throws GlobalException {
        ...
        params.put("sign", MD5.md5(account.getMerchantCode() + orderId + "orderquery" + account.getPrivateKey()))

        log.info("WayPayScript_query_params:{}", JSON.toJSONString(params))
        ...
        log.info("WayPayScript_query_resp:{}", resStr)
        if (StringUtils.isEmpty(resStr)) {
            return null
        }
