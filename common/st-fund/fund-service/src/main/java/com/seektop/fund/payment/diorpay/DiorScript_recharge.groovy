...
class DiorScript_recharge {

    private static final Logger log = LoggerFactory.getLogger(DiorScript_recharge.class)
    ...
    public void pay(Object[] args) {
        ...
        if (merchant.getPaymentId() == FundConstant.PaymentType.BANKCARD_TRANSFER) {
            ...
            service = "ALIPAY"
        } else if (merchant.getPaymentId() == FundConstant.PaymentType.ONLINE_PAY || merchant.getPaymentId() == FundConstant.PaymentType.QUICK_PAY) {
            service = "EXPRESS"
        }
    ...
    private void prepareScan(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount payment, RechargePrepareDO req, GlRechargeResult result, String service) {
        try {
            ...
            DataContentParams.put("Remark", req.getFromCardUserName())
            ...
            if (json.getString("type") == "url" || json.getString("type") == "img") {
                result.setRedirectUrl(json.getString("info"))
            } else if (json.getString("type") == "string") {
