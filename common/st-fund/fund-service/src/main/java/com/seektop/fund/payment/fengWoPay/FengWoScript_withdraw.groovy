 ...
 * @auth Otto
 */
public class FengWoScript_withdraw {


    private static final Logger log = LoggerFactory.getLogger(FengWoScript_withdraw.class)

    private OkHttpUtil okHttpUtil
    private GlPaymentChannelBankBusiness glPaymentChannelBankBusiness
    private static final String SERVER_WITHDRAW_URL = "/api/v1/transfers"
    private static final String SERVER_QUERY_URL = "/api/v1/transfers/query"
    private static final String SERVER_BALANCE_URL = "/api/v1/balance"
    ...
    public WithdrawResult withdraw(Object[] args) throws GlobalException {
        ...
        log.info("FengWoScript_doTransfer_params:{}", JSON.toJSONString(params))
        ...
        String resStr = okHttpUtil.postJSON(account.getPayUrl() + SERVER_WITHDRAW_URL, JSON.toJSONString(params), headParams, requestHeader)
        log.info("FengWoScript_doTransfer_resp:{}", resStr)
    ...
    public WithdrawNotify withdrawNotify(Object[] args) throws GlobalException {
        ...
        log.info("FengWoScript_doTransferNotify_resp:{}", JSON.toJSONString(resMap))
        ...
        if (StringUtils.isNotEmpty(orderId) || thirdSign == sign ) {
            return this.withdrawQuery(okHttpUtil, account, orderId, args[3])
        }

        log.info("FengWoScript_doTransferNotify_Sign 回调有误或验签失败 orderId :{}", orderId )
        return null
    }
    ...
    public WithdrawNotify withdrawQuery(Object[] args) throws GlobalException {
        ...
        log.info("FengWoScript_doTransferQuery_params:{}", JSON.toJSONString(params))
        GlRequestHeader requestHeader =
                this.getRequestHeard("", "", orderId, GlActionEnum.WITHDRAW_QUERY.getCode(), merchant.getChannelId(), merchant.getChannelName())
        String resStr = okHttpUtil.postJSON(merchant.getPayUrl() + SERVER_QUERY_URL, JSONObject.toJSONString(params), headParams, requestHeader)
        log.info("FengWoScript_doTransferQuery_resp:{}", resStr)
        if (StringUtils.isEmpty(resStr)) {
            return null
        }
        JSONObject json = JSON.parseObject(resStr)
        ...
        } else if (dataJSON.getString("status") == ("failed") || dataJSON.getString("status") == ("expired") ) {
            notify.setStatus(1)
            notify.setRsp("ok")
    ...
    public BigDecimal balanceQuery(Object[] args) throws GlobalException {
        ...
        log.info("FengWoScript_Query_Balance_Params: {}", JSON.toJSONString(params))
        GlRequestHeader requestHeader =
                getRequestHeard(null, null, null, GlActionEnum.PAYMENT_QUERY_BALANCE.getCode(),merchant.getChannelId(),merchant.getChannelName())
        String resStr = okHttpUtil.postJSON(merchant.getPayUrl() + SERVER_BALANCE_URL, JSONObject.toJSONString(params), headParams, requestHeader)
        log.info("FengWoScript_Query_Balance_resStr: {}", resStr)
