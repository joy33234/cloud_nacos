...
public class C2CWithdrawReceiveTimeoutCustomer {

    @Resource
    private C2COrderHandler c2COrderHandler;

    @RabbitListener(queues = QueueConstant.C2C_ORDER_RECEIVE_TIMEOUT_PROGRESS, containerFactory = "reportRabbitListener")
    public void receiveNotice(Channel channel, Message message) throws IOException {
        ...
        try {
            c2COrderHandler.submitReceiveTimeout(withdrawOrderId);
            channel.basicAck(deliveryTag, false);
        } catch (Exception ex) {
            log.error("C2C提现订单{}收款超时检查时发生异常", withdrawOrderId, ex);
        }
    }
