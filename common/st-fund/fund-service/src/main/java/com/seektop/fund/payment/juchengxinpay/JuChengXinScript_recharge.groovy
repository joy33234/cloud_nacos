...
class JuChengXinScript_recharge {

    private static final Logger log = LoggerFactory.getLogger(JuChengXinScript_recharge.class)
    ...
    private void prepareScan(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount payment, RechargePrepareDO req, GlRechargeResult result, String payType) {
        try {
            ...
            log.info("JuChengXinScript_Prepare_Params:{}", JSON.toJSONString(DataContentParms))
            ...
            log.info("JuChengXinScript_Prepare_resStr:{}", restr)
    ...
    RechargeNotify notify(Object[] args) throws GlobalException {
        ...
        log.info("JuChengXinScript_Notify_resMap:{}", JSON.toJSONString(resMap))
    ...
    RechargeNotify payQuery(Object[] args) throws GlobalException {
        ...
        log.info("JuChengXinScript_Query_reqMap:{}", JSON.toJSONString(DataContentParms))
        ...
        log.info("JuChengXinScript_Query_resStr:{}", resStr)

        JSONObject json = JSON.parseObject(resStr)

        //支付完成：1   等待付款：2    支付关闭：3。
        if (json != null && "1" == json.getString("ret_code")) {
