 ...
 * @date 2021-03-30
 */
class YonghengV3Script_recharge {

    private static final Logger log = LoggerFactory.getLogger(YonghengV3Script_recharge.class)
    ...
    private void prepareScan(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount payment, RechargePrepareDO req, GlRechargeResult result) {
        ...
        String toSign = JSONObject.toJSONString(paramMap) + payment.getPrivateKey()
        paramMap.put("pay_md5_sign", MD5.md5(toSign))
        // 网关及支付宝转银联需要实名
        ...
        log.info("YonghengV3Script_Prepare_Params: {}", JSON.toJSONString(paramMap))
        ...
        log.info("YonghengV3Script_Prepare_Resp: {}", json)
        if (json == null) {
    ...
    RechargeNotify notify(Object[] args) throws GlobalException {
        ...
        log.info("YonghengV3Script_Notify_resMap:{}", JSON.toJSONString(resMap))
    ...
    RechargeNotify payQuery(Object[] args) throws GlobalException {
        ...
        String toSign = JSON.toJSONString(paramMap) + payment.getPrivateKey()
        paramMap.put("pay_md5_sign", MD5.md5(toSign))

        String queryUrl = payment.getPayUrl() + "/api/query_transaction"
        log.info("YonghengV3Script_Query_Params: {}", JSON.toJSONString(paramMap))
        ...
        log.info("YonghengV3Script_Query_resStr: {}", result)
