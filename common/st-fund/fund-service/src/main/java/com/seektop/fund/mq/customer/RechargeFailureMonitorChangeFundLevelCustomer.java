...
import com.seektop.fund.handler.RechargeFailureMonitorHandler;
...
public class RechargeFailureMonitorChangeFundLevelCustomer {

    @Resource
    private RechargeFailureMonitorHandler rechargeFailureMonitorHandler;

    @RabbitListener(queues = QueueConstant.RECHARGE_FAILURE_MONITOR_CHANGE_FUND_LEVEL, containerFactory = "reportRabbitListener")
    public void receiveNotice(Channel channel, Message message) throws IOException {
        ...
        try {
            rechargeFailureMonitorHandler.failureChangeFundLevel(dataObj);
            channel.basicAck(deliveryTag, false);
        } catch (Exception ex) {
            log.error("处理充值失败监控更改用户财务层级时发生异常", ex);
        }
    }
