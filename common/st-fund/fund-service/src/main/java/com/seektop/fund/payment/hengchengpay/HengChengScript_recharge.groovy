...
class HengChengScript_recharge {

    private static final Logger log = LoggerFactory.getLogger(HengChengScript_recharge.class)
    ...
    private void prepareScan(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount payment, RechargePrepareDO req, GlRechargeResult result, String paymentType) {
        ...
        log.info("HengChengScript_Prepare_Params:{} ,url:{}", JSON.toJSONString(DataContentParms), payment.getPayUrl())
        ...
        log.info("HengChengScript_Prepare_resStr:{} , orderId:{}", restr, req.getOrderId())
        ...
        if (dataJSON == null || StringUtils.isEmpty(dataJSON.getString("redirect"))) {
            result.setErrorCode(FundConstant.RechargeErrorCode.PAYMENT)
            result.setErrorMsg(json.getString("message")==null? "商户未出码" : "商户异常:" + json.getString("message"))
            return
        }
    ...
    RechargeNotify notify(Object[] args) throws GlobalException {
        ...
        log.info("HengChengScript_Notify_resMap:{}", JSON.toJSONString(resMap))
        ...
        log.info("HengChengScript_notify_Sign: 回调资料错误或验签失败，orderId：{}", orderId)
        return null
    ...
    RechargeNotify payQuery(Object[] args) throws GlobalException {
        ...
        log.info("HengChengScript_Query_reqMap:{}", JSON.toJSONString(DataContentParms))
        ...
        log.info("HengChengScript_Query_resStr:{}", resStr)
        ...
        if (1 == dataJSON.getInteger("statusSt")) {
