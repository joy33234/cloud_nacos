 ...
 * @auth Otto
 */
public class JiajiaScript_recharge {

    private static final String SERVER_PAY_URL = "/addons/payment/order/api_info"
    private static final String SERVER_QUERY_URL = "/addons/payment/order/api_get_order"
    private static final Logger log = LoggerFactory.getLogger(JiajiaScript_recharge.class)
    ...
    private void prepareToScan(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount account, RechargePrepareDO req, GlRechargeResult result, String payType) throws GlobalException {
        ...
        //业务参数
        reqParam.put("out_trade_no",req.getOrderId())
        reqParam.put("name",req.getFromCardUserName())
        reqParam.put("pay_type", payType)
        reqParam.put("total_amount", req.getAmount().setScale(2, RoundingMode.DOWN).toString())
        reqParam.put("biz_content", JSON.toJSONString(reqParam))

        //公共参数
        reqParam.put("app_id", account.getMerchantCode())
        ...
        reqParam.put("sign", MD5.md5(account.getPrivateKey() + account.getMerchantCode() + stamptime));
        reqParam.put("stamptime", stamptime)
        reqParam.put("version", "1.0")
        reqParam.put("notify_url", account.getNotifyUrl() + merchant.getId())

        log.info("JiajiaScript_recharge_prepare_params:{}", JSON.toJSONString(reqParam))
        ...
        String resStr = okHttpUtil.postJSON(account.getPayUrl() + SERVER_PAY_URL, JSON.toJSONString(reqParam), requestHeader)
        log.info("JiajiaScript_recharge_prepare_resp:{}", resStr)
    ...
    public RechargeNotify notify(Object[] args) throws GlobalException {
        ...
        log.info("JiajiaScript_notify_resp:{}",resMap)
    ...
    public RechargeNotify payQuery(Object[] args) throws GlobalException {
        ...
        log.info("JiajiaScript_query_params:{}", JSON.toJSONString(params))
        ...
        log.info("JiajiaScript_query_resp:{}", resStr)
        if (StringUtils.isEmpty(resStr)) {
            return null
        }
        ...
        if ("1" == (dataJSON.getString("states")) || "2" == (dataJSON.getString("states"))) {
            ...
            pay.setRsp("{\"code\":1,\"msg\":\"Success\",\"data\":{\"orderid\":\""+orderId+"\"}}")
            return pay
