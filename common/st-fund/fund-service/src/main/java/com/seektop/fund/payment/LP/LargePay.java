 ...
 */
@Slf4j
@Service(FundConstant.PaymentChannel.LARGEPAY + "")
public class LargePay implements GlPaymentRechargeHandler, GlPaymentWithdrawHandler {
    ...
    public void prepare(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount account, RechargePrepareDO req, GlRechargeResult result) throws GlobalException {
        if (FundConstant.PaymentType.ONLINE_PAY == merchant.getPaymentId()) {
            prepareOnline(merchant, account, req, result);
        } else if (FundConstant.PaymentType.QUICK_PAY == merchant.getPaymentId()) {
            prepareToQuick(merchant, account, req, result);
        } else if (FundConstant.PaymentType.UNION_PAY == merchant.getPaymentId()) {
            prepareToUnionScan(merchant, account, req, result);
        }
    }
     ...
     * WEB端网银
     */
    private void prepareOnline(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount account, RechargePrepareDO req, GlRechargeResult result) {
        try {
            ...
            String bank_code = paymentChannelBankBusiness.getBankCode(req.getBankId(), account.getChannelId());//银行编号
            ...
            log.info("LargePay_Prepare_toSign:{}", toSign);
            ...
            log.info("large_recharge_prepareWangyin_params:{}", JSON.toJSONString(param));
            String retBack = HtmlTemplateUtils.getPost(account.getPayUrl() + "/gateway/pay.jsp", param);
            log.info("large_recharge_prepareWangyin_result:{}", retBack);
    ...
    //银联快捷
    private void prepareToQuick(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount account, RechargePrepareDO req, GlRechargeResult result) {
        try {
            String keyValue = account.getPrivateKey();//商户接口秘钥
            String version = "v1";//接口版本
            String merchant_no = account.getMerchantCode();//商户号
            String order_no = req.getOrderId();//商户订单号
            String goods_name = Base64.getEncoder().encodeToString("CZ".getBytes("utf-8"));//商品名称
            String order_amount = req.getAmount().setScale(2, RoundingMode.DOWN).toString(); //订单金额
            String backend_url = account.getNotifyUrl() + merchant.getId();//支付结果异步通知地址
            String frontend_url = account.getResultUrl() + merchant.getId();//支付结果同步通知地址
            String reserve = "";//商户保留信息
            String pay_mode = "07";//支付模式
            String bank_code = "QUICKPAY";//银行编号
            String card_type = "0";//允许支付的银行卡类型
            String merchant_user_id = req.getUserId().toString();//商户平台会员ID（建议用数据库中对应的会员数据记录的自增列（mysql）或序列(oracle)的值）
            String bank_card_no = req.getFromCardNo();//进行支付的银行卡号
            ...
            toSign.append("bank_code=").append(bank_code).append("&");
            toSign.append("card_type=").append(card_type).append("&");
            toSign.append("merchant_user_id=").append(merchant_user_id).append("&");
            toSign.append("key=").append(keyValue);
            log.info("LargePay_Prepare_toSign:{}", toSign);
            String sign = MD5.md5(toSign.toString());
            ...
            param.put("merchant_user_id", merchant_user_id);
            param.put("bank_card_no", bank_card_no);
            param.put("sign", sign);

            log.info("large_recharge_prepareQuick_params:{}", JSON.toJSONString(param));
            String retBack = HtmlTemplateUtils.getPost(account.getPayUrl() + "/gateway/pay.jsp", param);
            log.info("large_recharge_prepareQuick_result:{}", retBack);
            result.setMessage(retBack);
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException("创建订单失败");
    ...
    //银联扫码支付
    private void prepareToUnionScan(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount account, RechargePrepareDO req, GlRechargeResult result) {
        try {
            String keyValue = account.getPrivateKey();//商户接口秘钥
            String version = "v1";//接口版本
            String merchant_no = account.getMerchantCode();//商户号
            ...
            String frontend_url = account.getResultUrl() + merchant.getId();//支付结果同步通知地址
            String reserve = "";//商户保留信息
            String pay_mode = "12";//支付模式
            String bank_code = "UNIONPAYWAP";//银行编号
            String card_type = "0";//允许支付的银行卡类型
            goods_name = Base64.getEncoder().encodeToString(goods_name.getBytes("utf-8"));
            //MD5签名
            StringBuilder toSign = new StringBuilder();
            ...
            log.info("large_recharge_prepareUnion_params:{}", JSON.toJSONString(param));
            String retBack = HtmlTemplateUtils.getPost(account.getPayUrl() + "/gateway/pay.jsp", param);
            log.info("large_recharge_prepareUnion_result:{}", retBack);
            result.setMessage(retBack);
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException("创建订单失败");
    ...
    @Override
    public RechargeNotify result(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount
            account, Map<String, String> resMap) throws GlobalException {
        return notify(merchant, account, resMap);
    }

    @Override
    public RechargeNotify notify(GlPaymentMerchantApp merchant, GlPaymentMerchantaccount
            account, Map<String, String> resMap) throws GlobalException {
        log.info("LPPayer_Notify_resMap:{}", JSON.toJSONString(resMap));
    ...
    public RechargeNotify query(GlPaymentMerchantaccount account, String orderId) throws GlobalException {
        ...
        log.info("GPayer_query_param: {}", param);
        GlRequestHeader requestHeader = GlRequestHeader.builder()
                .action(GlActionEnum.DEPOSIT_QUERY.getCode())
                .channelId(PaymentMerchantEnum.LARGE_PAY.getCode() + "")
                .channelName(PaymentMerchantEnum.LARGE_PAY.getPaymentName())
                .userId("")
                .userName("")
                .tradeId(orderId)
                .build();
        ...
        log.info("GPayer_query_resStr: {}", resStr);
    ...
    public WithdrawResult doTransfer(GlWithdrawMerchantAccount account, GlWithdraw req) throws GlobalException {
        ...
        String bank_code = paymentChannelBankBusiness.getBankCode(req.getBankId(), account.getChannelId());//银行代码
        String bank_name = paymentChannelBankBusiness.getBankName(req.getBankId(), account.getChannelId());//银行名称
        try {
        ...
        param.put("sign", sign);
        log.info("LPPayer_doTransfer_param: {}", param);
        ...
        log.info("LPPayer_doTransfer_resStr: {}", resStr);
    ...
    public WithdrawNotify doTransferNotify(GlWithdrawMerchantAccount merchant, Map<String, String> resMap) throws
            GlobalException {
        log.info("LPPayer_Transfer_Notify_resMap:{}", JSON.toJSONString(resMap));
    ...
    @Override
    public WithdrawNotify doTransferQuery(GlWithdrawMerchantAccount account, String orderId) throws
            GlobalException {
        ...
        log.info("LargePay_TransferQuery_reqMap:{}", param);
        GlRequestHeader requestHeader = GlRequestHeader.builder()
        ...
        log.info("LargePay_TransferQuery_resStr:{}", resStr);
        JSONObject json = JSON.parseObject(resStr);
        if (json == null) {
            return null;
        }
    ...
    public BigDecimal queryBalance(GlWithdrawMerchantAccount account) throws GlobalException {
        ...
        log.info("LPPayer_QueryBalance_resStr:{}", resStr);
