...
import com.seektop.common.annotation.STApiRequestComponent;
import com.seektop.common.annotation.STApiRequestComponentScan;
import com.seektop.common.utils.HsfTypeFilter;
 ...
 */
@Slf4j
public class STApiRequestBeanDefinitionRegistrar implements ImportBeanDefinitionRegistrar {
    ...
    public void registerBeanDefinitions(AnnotationMetadata importingClassMetadata, BeanDefinitionRegistry registry) {
        //拿到主类上的自定义注解的属性
        AnnotationAttributes annAttr = AnnotationAttributes.fromMap(importingClassMetadata.getAnnotationAttributes(STApiRequestComponentScan.class.getName()));
        ...
        if (candidates.isEmpty()) {
            log.info("扫描指定HSF基础包[{}]时未发现复合条件的基础类", Arrays.toString(basePackages));
            return;
        }
        //注册处理器后,为 对象注入环境配置信息
        //通过该类对对象进行进一步操作
    ...
    private void registerBeanDefinitions(List<Class<?>> internalClasses, BeanDefinitionRegistry registry) {
        for (Class<?> clazz : internalClasses) {
            ...
            String value = clazz.getAnnotation(STApiRequestComponent.class).value();

            if(StringUtils.isEmpty(value)){
                value = ClassUtils.getShortNameAsProperty(clazz);
            }
            definition.getPropertyValues().add("value", value);
            definition.setBeanClass(STApiRequestFactoryBean.class);
    ...
    private boolean registerSpringBean(Class<?> beanClass) {
        return beanClass.getAnnotation(STApiRequestComponent.class).registerBean();
    }
