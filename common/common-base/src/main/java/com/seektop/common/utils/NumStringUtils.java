...
public class NumStringUtils {
     ...
     * 将获取到的数字字符串加上分隔符导出
     */
    public static String csvExportSeparator(String string) {
        BigDecimal decimal = string2Decimal(string);
        DecimalFormat df2 = new DecimalFormat("###,##0.00");
        String format = df2.format(decimal);
        return "\"" + format + "\"";
    }
    ...
    public static BigDecimal string2Decimal(String str) {
        ...
        try {
            bigDecimal = BigDecimal.valueOf(Double.parseDouble(str)).setScale(2, BigDecimal.ROUND_DOWN);
     ...
     * 生成随机字符串（大小写字母加数字组合）
     *
     * @param targetStringLength 需要的字符串长度
     * @return
     */
